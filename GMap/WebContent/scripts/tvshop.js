// JavaScript Document
function XoaHinhMoTa(stt) {
	document.forms["frmDangSanPham"]["hanhDong"].value = 'xoaHinh';
	document.forms["frmDangSanPham"]["xoaHinh"].value = stt;
	document.forms["frmDangSanPham"].submit();
}
function TestFileType( fileTypes ) {
	var fileName = document.forms["frmDangSanPham"]["hinhSanPham"].value;
	if (!fileName) return;	
	dots = fileName.split(".");
...
function callThemHinhSubmit(){
	//if (document.forms["frmDangSanPham"]['hinhSanPham'].value.length <= 0) return;
	document.forms["frmDangSanPham"]["hanhDong"].value = 'themHinh';
	document.forms["frmDangSanPham"].submit();
}
function callThemSanPhamSubmit(){
	if (!checkValidationUpload()) return;
	document.forms["frmDangSanPham"]["hanhDong"].value = 'themSanPham';
	document.forms["frmDangSanPham"].submit();
}

function clickCapNhat(){
	document.forms["frmGioHang"]["hanhDong"].value = 'capNhat';
	document.forms["frmGioHang"].submit();
}

function clickThanhToan(){
	document.forms["frmGioHang"]["hanhDong"].value = 'thanhToan';
	document.forms["frmGioHang"].submit();
}
...
function checkValidationUploadObj(objName) {
	var x=document.forms["frmDangSanPham"][objName].value;
	if (x==null || x.length<=4) {
		document.forms["frmDangSanPham"][objName].className = 'imgCheckError';
		return false;
	} else {
		document.forms["frmDangSanPham"][objName].className = 'imgCheckOk';
		return true;		
	}
}

function checkValidationUploadNumberObj(objName) {
	var x=document.forms["frmDangSanPham"][objName].value;
	if (x==null || x.length<=0 || parseInt(x)==NaN || parseInt(x)<=0 ) {
		document.forms["frmDangSanPham"][objName].className = 'imgCheckError';
		return false;
	} else {
		document.forms["frmDangSanPham"][objName].className = 'imgCheckOk';
...
function checkValidationUploadSelectObj(objName) {
	var x=document.forms["frmDangSanPham"][objName].value;
	if (x==null || x.length<=0 || parseInt(x)==NaN || parseInt(x)<=0 ) {
		//document.forms["frmDangSanPham"][objName].className = 'imgCheckError';
		return false;
...
function checkValidationUpload() {
	var b = checkValidationUploadObj('tenSanPham');
	b = checkValidationUploadObj('nhaSanXuat')&&b;
	b = checkValidationUploadObj('trongLuong')&&b;
	b = checkValidationUploadNumberObj('soLuongTon')&&b;
	b = checkValidationUploadNumberObj('giaBan')&&b;
	b = checkValidationUploadSelectObj('maDanhMuc')&&b;
	return b;
}
